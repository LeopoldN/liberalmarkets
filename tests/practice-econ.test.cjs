...
test('all countries cycle without repeats and clues never rely on unavailable facts',()=>{
  ...
  for(let i=0;i<game.countries.length;i++) {
    const round=game.next();assert.equal(round.clues.length,2);assert.notEqual(round.clues[0].key,round.clues[1].key);
    ...
    for(const f of round.clues) assert.ok(!round.country.missing.includes(f.key));
  }
  assert.notEqual(game.next().country.iso3,last);
});
test('wrong, duplicate, invalid, correct and reveal transitions',()=>{
  const game=createGame(data.countries,()=>.5);const round=game.next();const wrong=game.countries.find(c=>c.iso3!==round.country.iso3);
